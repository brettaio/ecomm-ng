...
export class PortfolioMasonryFiveComponent implements OnInit {
  private _album: { src: string; caption: string; thumb: string }[] = [];
  ...
  constructor(private _lightbox: Lightbox) {
    for (let i = 13; i <= 24; i++) {
      ...
      this._album = this.list.map((item) => ({
        src: item.image,
        caption: item.title + " caption here",
        thumb: item.image.replace(".jpg", "-thumb.jpg"),
      }));
    }
  }
